...
%
% References: Polyachenko & Shukhman (2025)
%
...
% Doubling factors: 1 at alpha=0, 2 for alpha>0 (to represent [-inf,inf])
% twofac = ones(1, N_alpha);
% if abs(alpha_arr(1)) < 1e-14, twofac(1) = 1; end
% for i=2:N_alpha, twofac(i) = 2; end
% S_alpha2 = S_alpha .* twofac;  % use S_alpha2 wherever alpha-columns are weighted
...
% Model selection
model = 9;  % 1: Toomre-Zang n=4, m=2;  9: Isochrone JH
    ...
    for ie = 1:Ne
        ...
        for i_l = 1:n_l
            ...
            for i_alpha = 1:N_alpha
                ...
                % Integrate using Trapezoid rule over non-uniform w1 (factor 1/pi)
...
% From Eqs. 21, 22, 24 in the paper (with ansatz F_l = int C_l W_l^*):
% D_l(beta, alpha) = integral dJ W_l(J, beta) W_l^*(J, alpha)  [Hermitian]
% E_l(beta, alpha) = integral dJ [l*Omega_1 + m*Omega_2] W_l(J, beta) W_l^*(J, alpha)  [Hermitian]
% F_l(beta, alpha) = integral dJ F_{0,l}(J) W_l(J, beta) W_l^*(J, alpha)  [same structure]
%
% where F_{0,l} = l * dF0/dJ + m * dF0/dL

% Compute F_{0,l} = l * FE + m * FL (using chain rule dF/dJ ~ dF/dE * dE/dJ)
% For DF_type = 0, FE = dF/dE, FL = dF/dL
% Need: dE/dJ at fixed L = Omega_1, dL/dJ = 0
% So F_{0,l} for each l requires proper transformation

% Actually, from the paper: F_{0,l} = l * dF0/dJ + m * dF0/dL
...
for i_l = 1:n_l
    ...
    % for i_beta = 1:N_alpha
    %     for i_alpha = 1:N_alpha
    %         % Integrate over phase space (Rc, v)
    %         D_sum = 0;
    %         E_sum = 0;
    %         F_sum = 0;
    % 
    %         for iR = 1:NR
    %             for ie = 1:Ne
    %                 W_beta = W_l(iR, ie, i_beta, i_l);
    %                 W_alpha = W_l(iR, ie, i_alpha, i_l);
    % 
    %                 if W_beta == 0 || W_alpha == 0
    %                     continue;
    %                 end
    % 
    %                 % Integration weight: dJ dL = Jac * dRc dv
    %                 % So: S_RC(iR) * S_e(iR, ie) * Jac(iR, ie)
    %                 weight = S_RC(iR) * S_e(iR, ie) * Jac(iR, ie);
    % 
    %                 if weight == 0
    %                     continue;
    %                 end
    % 
    %                 % All use W(beta) * W^*(alpha) - Hermitian structure
    %                 WW = W_beta * conj(W_alpha) * weight;  % W(beta) * W^*(alpha)
    % 
    %                 D_sum = D_sum + WW;
    %                 E_sum = E_sum + (l * Omega_1(iR, ie) + m * Omega_2(iR, ie)) * WW;
    % 
    %                 % F_{0,l} = (l*Omega_1 + m*Omega_2) * FE + m * FL
    %                 F0l = (l * Omega_1(iR, ie) + m * Omega_2(iR, ie)) * FE(iR, ie) + m * FL(iR, ie);
    %                 F_sum = F_sum + F0l * WW;
    %             end
    %         end
    % 
    %         D_l_mat(i_beta, i_alpha, i_l) = D_sum;
    %         E_l_mat(i_beta, i_alpha, i_l) = E_sum;
    %         F_l_mat(i_beta, i_alpha, i_l) = F_sum;
    %     end
    % end
...
% B_{l,l'}(beta, alpha') = G * integral_{alpha>=0} [2-\delta_{alpha0}] d_alpha/(2*pi) N(alpha,m) F_l(beta, alpha) D_{l'}(alpha, alpha')
% This exploits the separable kernel structure (Eq. 26)

G = 1;  % Gravitational constant (set to 1 in dimensionless units)

B_mat = zeros(N_alpha * n_l, N_alpha * n_l);

for i_l = 1:n_l
    for i_lp = 1:n_l
        % B_sum_mat = zeros(N_alpha);
        % 
        % for i_beta = 1:N_alpha
        %     for i_alphap = 1:N_alpha
        %         % Integrate over alpha/(2*pi) using Simpson weights (Eq. 26)
        %         B_sum = 0;
        %         for i_alpha = 1:N_alpha
        %             % alpha>=0 folding: multiply positive nodes by 2 (twofac)
        %             B_sum = B_sum + S_alpha(i_alpha) * N_kernel(i_alpha) * ...
        %                 F_l_mat(i_beta, i_alpha, i_l) * D_l_mat(i_alpha, i_alphap, i_lp);
        %         end
        % 
        %         % Global index: (l, alpha) -> linear index
        %         idx_row = (i_l - 1) * N_alpha + i_beta;
        %         idx_col = (i_lp - 1) * N_alpha + i_alphap;
        % 
        %         B_mat(idx_row, idx_col) = G * B_sum ; %* S_alpha2(i_alphap);
        %     end
        % end
...
for i_l = 1:n_l
    ...
    % Include quadrature weights for d_alpha integration (Eq. 27)
    D_global(idx_start:idx_end, idx_start:idx_end) = D_l_mat(:, :, i_l);% * diag(S_alpha2);
    E_global(idx_start:idx_end, idx_start:idx_end) = E_l_mat(:, :, i_l);% * diag(S_alpha2);

    PR_D = PR_D * det(D_l_mat(:, :, i_l));
    PR_E = PR_E * det(E_l_mat(:, :, i_l));
    PR_F = PR_F * det(F_l_mat(:, :, i_l));
end
disp(num2str([PR_D, PR_E, PR_F], "det(D)=%.f, det(E)=%.f, det(F)=%.f"))
...
I = 1:N_alpha;
for i_l = 1:n_l, 
    PR_D = PR_D * det(D_global((i_l-1)*N_alpha +I, (i_l-1)*N_alpha +I)); 
    PR_E = PR_E * det(E_global((i_l-1)*N_alpha +I, (i_l-1)*N_alpha +I)); 
end
disp(num2str([PR_D, PR_E], "det(D)=%.f, det(E)=%.f"))
