% ZANG_NEWTON_SEARCH_MP_PARALLEL
% ---------------------------------------------------------------
% Newton-Raphson search to find (Ω_p, s) such that λ(ω) = 1
% for the m=2 mode of the Zang disk.
%
% MULTIPRECISION PARALLEL VERSION using Advanpix Toolbox (quadruple precision)
% with parfor parallelization for speedup
%
...
fprintf('Precomputing orbits and Q_{ℓm}(α;η) in parallel...\n');
tic;
orbitData = cell(1,nEta);

% PARALLELIZED: Orbit computation
parfor k = 1:nEta
    orbitData{k} = compute_orbit_data(eta(k), NW_orbit, NW);
end
orbit_time = toc;
fprintf('  Orbit computation: %.2f seconds\n', orbit_time);

% PARALLELIZED: Q_lm computation
tic;
Q_lmk = compute_Q_lm_parallel(alpha, eta, orbitData, m, ellVec);
qlm_time = toc;
fprintf('  Q_lm computation: %.2f seconds\n', qlm_time);
...
% ---------- Precompute h-grid quantities (expensive, do once) ----------
fprintf('Precomputing h-grid and exp_matrix...\n');
tic;
hmax = mp('4.0');
Nh   = 8192;
...
dAlpha_scalar = alpha(2) - alpha(1);
nuVals = (-nAlpha+1:nAlpha-1) * dAlpha_scalar;
nuVals_col = nuVals(:);
h_row      = h(:).';
exp_matrix = exp(-mp('1i') * nuVals_col * h_row) .* repmat(dh, length(nuVals_col), 1);

% Precompute index matrix for kernel building
[JJ, II] = meshgrid(1:nAlpha, 1:nAlpha);
idxMat = (II - JJ) + nAlpha;
idxMat_flat = idxMat(:);

% Precompute orbit-derived vectors
...
hgrid_time = toc;
fprintf('  h-grid precomputation: %.2f seconds\n', hgrid_time);
...
% h-grid precomputed data
precomp.Hhat_h       = Hhat_h;
precomp.Hhat_prime_h = Hhat_prime_h;
precomp.exp_h        = exp_h;
precomp.exp_matrix   = exp_matrix;
precomp.idxMat_flat  = idxMat_flat;
precomp.weight_vec   = weight_vec;
precomp.kappa_vec    = kappa_vec;
precomp.Omega_vec    = Omega_vec;
...
for iter = 1:max_iter
    ...
    % PARALLELIZED: Compute λ at current point AND Jacobian perturbations in parallel
    % This computes 3 eigenvalues in one parallel batch
    fprintf('  [Iter %d] Computing 3 eigenvalues (each needs 200 eta loops)...\n', iter);
    omega_all = [omega; 
                 m*(Omega_p + delta) + mp('1i')*s; 
                 m*Omega_p + mp('1i')*(s + delta)];
    lambda_all = zeros(3, 1, 'mp');
    
    eig_tic = tic;
    parfor i = 1:3
        lambda_all(i) = compute_eigenvalue(omega_all(i), precomp);
    end
    eig_time = toc(eig_tic);
    fprintf('  [Iter %d] Eigenvalues computed in %.1fs (%.1fs per eigenvalue)\n', iter, eig_time, eig_time/3);
    
    lambda = lambda_all(1);
    lambda_dOp = lambda_all(2);
    lambda_ds = lambda_all(3);
    ...
    fprintf('Iter %2d: Ω_p=%.16f  s=%.16f  λ=%.16f%+.16fi  |F|=%.2e  [%.2fs, total %.1fs]\n', ...
            iter, double(Omega_p), double(s), double(real(lambda)), double(imag(lambda)), double(res_norm), iter_time, total_iter_time);
...
Sm = build_kernel_parallel(omega, precomp);
...
% =================================================================
% Compute Q_{ℓm}(α;η) - PARALLELIZED VERSION
% =================================================================
function Q = compute_Q_lm_parallel(alpha, eta, orbitData, m, ellVec)
nAlpha = numel(alpha);
nEta   = numel(eta);
nEll   = numel(ellVec);
...
% Parallelize over eta (outer loop) with FULLY VECTORIZED alpha loop
parfor kEta = 1:nEta
    orb  = orbitData{kEta};
    X    = orb.X(:).';
    Y    = orb.Y(:).';
    ...
    % Vectorize over all alpha values at once
    % alpha is (1 x nAlpha), X is (1 x nPsi)
    % exp_term: (nAlpha x nPsi)
    alpha_col = alpha(:);
    exp_term = exp((mp('1i') * alpha_col - mp('0.5')) * X);  % (nAlpha x nPsi)
    base_kernels = exp_term .* repmat(dpsi, nAlpha, 1);  % (nAlpha x nPsi)
    
    % ell_phase: (nEll x nPsi) - same for all alpha
    ell_phase = cos(ell_col * psi - m * repmat(Y, nEll, 1));  % (nEll x nPsi)
    
    % Q_slice(iEll, ia) = sum over psi of ell_phase(iEll,:) .* base_kernel(ia,:)
    % This is ell_phase * base_kernels.' / pi, giving (nEll x nAlpha)
    Q_slice = (ell_phase * base_kernels.') / mp('pi');  % (nEll x nAlpha)
...
% =================================================================
% Build kernel S_m(β,α) - PARALLELIZED VERSION
% =================================================================
function Sm = build_kernel_parallel(omega, precomp)
% Uses precomputed data from precomp struct to avoid recomputation

alpha      = precomp.alpha;
Kalpha     = precomp.Kalpha;
Q_lmk      = precomp.Q_lmk;
ellVec     = precomp.ellVec;
a_par      = precomp.a_par;
m          = precomp.m;
Hhat_h       = precomp.Hhat_h;
Hhat_prime_h = precomp.Hhat_prime_h;
exp_h        = precomp.exp_h;
exp_matrix   = precomp.exp_matrix;
idxMat_flat  = precomp.idxMat_flat;
weight_vec   = precomp.weight_vec;
kappa_vec    = precomp.kappa_vec;
Omega_vec    = precomp.Omega_vec;

nAlpha = numel(alpha);
nEta   = numel(precomp.eta);
nEll   = numel(ellVec);

% PARALLELIZED: eta loop
Sm_parts = cell(nEta, 1);
...
parfor kEta = 1:nEta
    ...
    Sm_parts{kEta} = weight_vec(kEta) * Sm_eta;
end
