...
nAlpha   = 501;       % must be odd; increase for better resolution
alphaMax = 12.0;     % |α| ≤ alphaMax; increase if needed
alpha    = linspace(-alphaMax, alphaMax, nAlpha);
% dAlpha   = alpha(2)-alpha(1);
dAlpha   = NewtonCotes(2, alpha);

% Eccentric-velocity grid η = U/V0
nEta   = 250;         % increase for better convergence
...
elseif eta< 0.01
    ...
    % Compute half orbit (w: 0 to pi), then mirror
    nHalf = nPsi/2 + 1;
    w = linspace(0, pi, nHalf);
    cos_w = cos(w);
    sin_w = sin(w);
    ...
    t_full = [t_half 2*T_half-fliplr(t_half(1:end-1))];
    ...
    Y_half = phi_half - Omega_bar * t_half;

    % Mirror to full orbit:
    % X is symmetric (r retraces same values)
    % Y: by symmetry, Y(psi) = 2*Y(pi) - Y(2*pi - psi) for second half
    X = [X_half(1:end-1), fliplr(X_half(2:end))];
    Y = [Y_half(1:end-1), 2*Y_half(end) - fliplr(Y_half(2:end))];
    psi = t_full * kappa_bar;
    psi = psi(1:end-1);
...
for kEta = 1:nEta
    ...
    ell_phase = exp(-1i * ell_col * psi).*dpsi;
    Q(:,:,kEta) = (1/(2*pi)) * (ell_phase * kernel_alpha.');
end
end
