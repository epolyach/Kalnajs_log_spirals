...
const HELP_TEXT = """
Kalnajs Log-Spiral Single GPU Solver - Standalone Version

Usage: julia --project=. NL_julia_direct_gpu.jl [options]
...
Examples:
  julia --project=. NL_julia_direct_gpu.jl --config=configs/default.toml
  julia --project=. NL_julia_direct_gpu.jl --config=configs/large.toml --threads=4
"""
...
# ==================== GPU SETUP ====================
println("=" ^ 60)
println("KALNAJS LOG-SPIRAL SINGLE GPU SOLVER")
println("=" ^ 60)
println("Config:    $config_file")
println("Precision: ", USE_FLOAT32 ? "Float32" : "Float64")
println("BLAS threads: $max_threads")
println("GPU: ", CUDA.name(CUDA.device()), " (", round(CUDA.total_memory()/1024^3, digits=1), " GB)")
println()
...
# ==================== GPU TRANSFER ====================
println("\nTransferring to GPU...")
d_W_l_mat = CuArray(W_l_mat)
d_DJ_vec = CuArray(DJ_vec)
d_F0l_all = CuArray(F0l_all)
d_Omega_res = CuArray(Omega_res)
d_N_kernel = CuArray(N_kernel)
d_eyeN = CuArray(Matrix{CT}(I, N_alpha, N_alpha))

function compute_det(omega)
    omega_ct = CT(omega)
    M = CUDA.zeros(CT, N_alpha, N_alpha)
    for i_l in 1:n_l
        W = view(d_W_l_mat, :, :, i_l); denom = omega_ct .- view(d_Omega_res, :, i_l)
        denom = ifelse.(abs.(denom) .< FT(1e-12), CT(1e-12 + 1e-12im), denom)
        weight = d_DJ_vec .* view(d_F0l_all, :, i_l) ./ denom
        M .+= transpose(W) * (weight .* conj.(W))
    end
    M .*= G; M .*= reshape(d_N_kernel, 1, :)
    return det(CT.(Array(d_eyeN - M)))
end
